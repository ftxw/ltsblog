...
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      ...
      try { await recordLoginLog({ request, userId: "", username, summary: "登录失败：用户名或密码错误" }); } catch {}
      return NextResponse.json(
    ...
    const pwdMatch = await verifyPassword(password, user.hashed_password);
    if (!pwdMatch) {
