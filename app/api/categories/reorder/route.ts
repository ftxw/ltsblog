  ...
  try {
    ...
    if (Array.isArray(body.ids)) {
      ...
      await prisma.$transaction(
        full.map((id, idx) =>
          prisma.category.update({ where: { id }, data: { sort: idx + 1 } })
        )
      );
    ...
    if (i === -1 || j === -1) {
      return NextResponse.json({ code: 1, message: "分类不存在" }, { status: 404 });
    }
    [list[i], list[j]] = [list[j], list[i]];
    await prisma.$transaction(
      list.map((x, idx) =>
        prisma.category.update({ where: { id: x.id }, data: { sort: idx + 1 } })
      )
    );
    invalidateCatalogCaches();
    return NextResponse.json({ code: 0, message: "success" });
